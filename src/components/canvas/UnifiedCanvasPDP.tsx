...
const UnifiedCanvasPDP: React.FC<UnifiedCanvasPDPProps> = ({
  ...
  // Calculate dynamic minimum required markup to keep Creator Profit > $0.00
  const minMarkup = useMemo(() => {
    if (!platformMinSellingPrice || isNaN(platformMinSellingPrice) || platformMinSellingPrice <= 0) return 1;
    // We want creatorProfit = platformMinSellingPrice * (markup / 100) >= 0.01
    // markup >= 1 / platformMinSellingPrice
    return Math.max(1, Math.ceil(1 / platformMinSellingPrice));
  ...
  const creatorProfit = Math.round((minSellingPrice - platformMinSellingPrice) * 100) / 100;
  ...
  const platformFeeVal = minSellingPrice * 0.049;
  const platformFeeMaxVal = maxSellingPrice * 0.049;
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {uniqueSizes.map((size: string) => {
                      ...
                      const sizeVariants = selectedProduct?.variants?.filter((v: any) => v.size === size) || [];
                      const sizePrices = sizeVariants
                        .map((v: any) => parseFloat(v.price || '0'))
                        .filter((p: number) => p > 0);
                ...
                <div className="mt-4 bg-black/60 border border-white/5 p-4 rounded-2xl space-y-3 text-[11px] backdrop-blur-md">
                  ...
                  <div className="space-y-1.5 pt-1">
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-300">
                        {hasPriceRange ? `$${baseCostVal.toFixed(2)} - $${baseCostMaxVal.toFixed(2)}` : `$${baseCostVal.toFixed(2)}`}
                      </span>
                    </div>
                    <div className="flex justify-between border-b border-dashed border-white/5 pb-1 mb-1">
                      <span className="text-gray-500">Loka Base Price (0% Markup):</span>
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-500">Platform Fee (4.9%):</span>
                      <span className="text-gray-300">
                        {hasPriceRange ? `$${platformFeeVal.toFixed(2)} - $${platformFeeMaxVal.toFixed(2)}` : `$${platformFeeVal.toFixed(2)}`}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-500">Estimated Taxes (8%):</span>
                      <span className="text-gray-300">
                        {hasPriceRange ? `$${taxesVal.toFixed(2)} - $${taxesMaxVal.toFixed(2)}` : `$${taxesVal.toFixed(2)}`}
                      </span>
                    </div>
                    <div className="flex justify-between font-semibold border-t border-dashed border-white/10 pt-1.5 mt-1.5">
                      <span className="text-gray-400">Total Hidden Charges:</span>
                      <span className="text-yellow-500">
                        {hasPriceRange ? `$${totalHiddenChargesVal.toFixed(2)} - $${totalHiddenChargesMaxVal.toFixed(2)}` : `$${totalHiddenChargesVal.toFixed(2)}`}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-white/5 mt-1">
                    <div className="flex flex-col">
                      <span className="text-gray-400 font-semibold">Creator Profit ({effectiveMarkup}% Markup):</span>
                      <span className="text-[9px] text-gray-500">Calculated as Final Price - Loka Base Cost</span>
                    </div>
