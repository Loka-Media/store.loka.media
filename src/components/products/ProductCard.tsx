...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${createProductSlug(product.name, product.id)}`}>
      <div className="group relative bg-white border border-black rounded-2xl overflow-hidden hover:shadow-[8px_8px_0_0_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all duration-300 cursor-pointer">
        {/* Image Container */}
        <div className="relative overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100" style={{ aspectRatio: '1/1' }}>
          ...
          <div className="absolute top-4 right-4 z-10" onClick={(e) => e.stopPropagation()}>
            <button
              className={`p-2.5 rounded-full transition-all duration-300 focus:outline-none border border-black hover:shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] ${
                isWishlisted
        ...
        <div className="p-5">
          ...
          {product.category && (
            <div className="flex flex-wrap gap-2">
              <span className="inline-flex items-center bg-yellow-300 text-black text-xs px-3 py-1.5 rounded-full font-extrabold tracking-tight border border-black">
                {product.category}
              </span>
              {product.product_source && (
                <span
                  className={`inline-flex items-center text-xs px-3 py-1.5 rounded-full font-extrabold border border-black ${
