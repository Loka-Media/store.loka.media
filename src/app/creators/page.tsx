          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6 lg:gap-8">
            {filteredCreators.map((creator, index) => {
              ...
              return (
                ...
                >
                  <div className="w-full aspect-[3/4.4] min-h-[250px] sm:min-h-[380px] rounded-2xl sm:rounded-[2rem] overflow-hidden relative border border-white/15 hover:border-orange-500/40 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_45px_rgba(255,109,31,0.2)] flex flex-col bg-neutral-950">
                    ...
                    <div className="relative flex-1 w-full overflow-hidden">
                      {creatorImage ? (
                        ...
                        <div className="absolute inset-0 bg-gradient-to-br from-neutral-800 via-neutral-900 to-neutral-950 flex flex-col items-center justify-center transition-transform duration-700 group-hover:scale-110">
                          ...
                          <User className="w-10 h-10 sm:w-16 sm:h-16 text-white/5 opacity-50 blur-[0.5px] transition-transform duration-500 group-hover:scale-110" />
                        </div>
                      )}
                    ...
                    <div className="relative p-3 sm:p-6 bg-black/60 backdrop-blur-md border-t border-white/10 z-20 flex flex-col justify-end text-left min-h-[115px] sm:min-h-[160px] transition-all duration-500 group-hover:bg-black/75">
                      
                      {/* Name & Verified Check */}
                      <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1.5 min-w-0">
                        <span className="text-sm sm:text-xl font-extrabold text-white tracking-tight drop-shadow-md group-hover:text-orange-400 transition-colors duration-300 truncate">
                          {creator.name}
                        </span>
                        
                        {/* Verified badge check */}
                        <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-sky-400 fill-current flex-shrink-0 drop-shadow" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                          <path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                        </svg>
                      </div>

                      {/* Tagline / Username info */}
                      <p className="text-[10px] sm:text-xs text-white/70 font-semibold mb-2 sm:mb-4 drop-shadow truncate">
                      ...
                      <div className="flex items-center justify-between pt-2 sm:pt-4 border-t border-white/10 w-full mt-1 sm:mt-2 z-10 relative">
                        ...
                        <div className="px-2.5 py-1 sm:px-4 sm:py-2 rounded-full bg-white text-black font-black text-[9px] sm:text-[11px] uppercase tracking-wider group-hover:bg-gradient-to-r group-hover:from-orange-500 group-hover:to-pink-500 group-hover:text-white transition-all duration-300 shadow-md shrink-0">
                          Shop →
                        </div>
                      </div>
