...
const registerSchema = z
  ...
  .refine(
    (data) => {
      if (
        data.role === "creator" &&
        (!data.creatorUrl || data.creatorUrl.trim() === "")
      ) {
        return false;
      }
      return true;
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <div className="mb-4 sm:mb-6 lg:mb-8">
                <GradientTitle text="Sign Up" size="sm" />
              ...
              {/* Role Selection - Button Based */}
              <div className="flex gap-2 sm:gap-4">
                <div className="flex-1">
                  <button
                    type="button"
                    onClick={() => setValue("role", "creator")}
                    className={`w-full py-2 sm:py-3 px-3 sm:px-4 rounded-lg sm:rounded-xl font-bold text-sm sm:text-base transition-all border-2 ${
                      selectedRole === "creator"
                        ? "bg-orange-500 text-black border-orange-500"
                        : "bg-transparent text-orange-500 border-white/30 hover:border-white/50"
                    }`}
              ...
              {selectedRole === "creator" && (
                <div className="flex justify-end items-center gap-1.5 sm:gap-2">
                  <span className="text-xs sm:text-xs text-white/60">Creator Account Approval Required</span>
                  <div className="group relative cursor-help">
                    <AlertCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-orange-500" />
                    <div className="absolute bottom-full right-0 mb-2 w-40 sm:w-48 bg-black border border-white/20 rounded-lg p-2 sm:p-3 text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
